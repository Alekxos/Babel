...
class CloudSpeechClient:
    ...
    def recognize(self, language_code='en-US', hint_phrases=None, test_func=None):
        streaming_config=speech.types.StreamingRecognitionConfig(
            config=self._make_config(language_code, hint_phrases),
            single_utterance=True)
        print('test func version')
        ...
        with Recorder() as recorder:
            ...
            for response in responses:
                if response.speech_event_type == END_OF_SINGLE_UTTERANCE or (test_func is not None and test_func() == False):
                    recorder.done()
